...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    Args:
        feature_matrix - A numpy matrix describing the given data. Each row
            represents a single data point.--> shape = (N,d)
        labels - A numpy array where the kth element of the array is the
            correct classification of the kth row of the feature matrix. shape=(N)
        theta - A numpy array describing the linear classifier.
        theta_0 - A real valued number representing the offset parameter.
    ...
    z = y*(np.dot(feature_matrix,theta)+theta_0)
    loss_h = np.sum(1-z[z<1]) /size
    return loss_h
    raise NotImplementedError
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            ...
            cached_0+=theta_0
            count += 1
    return (cached/count, cached_0/count)
    raise NotImplementedError
